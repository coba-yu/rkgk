...
def test_the_structured_output_is_returned_as_the_payload(tmp_path: Path) -> None:
    ...
    assert ClaudeCodeAgent(command=command).answer("prompt", SCHEMA) == {"answer": "hello"}


def test_the_prompt_is_passed_on_standard_input(tmp_path: Path) -> None:
    command = write_fake_claude(tmp_path, json.dumps(SUCCESS))
    ClaudeCodeAgent(command=command).answer("a prompt", SCHEMA)
...
def test_a_model_is_passed_on_when_it_is_given(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(subprocess, "run", _fake_run)
    ClaudeCodeAgent(model="claude-opus-4").answer("prompt", SCHEMA)
...
def test_a_missing_command_is_reported(tmp_path: Path) -> None:
    with pytest.raises(StructuredOutputAgentError, match="was not found"):
        ClaudeCodeAgent(command=str(tmp_path / "absent")).answer("prompt", SCHEMA)
...
def test_a_command_that_never_answers_is_reported(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(StructuredOutputAgentError, match="did not answer within 1.5 seconds"):
        ClaudeCodeAgent(timeout_seconds=1.5).answer("prompt", SCHEMA)


@pytest.mark.slow
def test_the_real_claude_cli_answers_with_the_requested_structure() -> None:
    payload = ClaudeCodeAgent().answer("Answer with the word hello.", SCHEMA)
